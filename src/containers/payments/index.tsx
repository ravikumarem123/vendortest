...
const getPaymentDetailsPayload = ({
    ...
    return {
        ...
        startTime:
            dateClicked && fromDate && !paymentError
                ? dayjs(fromDate).startOf('day').valueOf()
                : null,
        endTime:
            dateClicked && toDate && !paymentError
...
const Payments = () => {
    ...
    const fetchData = ({ page, isDateClicked = false }: IFetchDataProps) => {
        const payload = getPaymentDetailsPayload({
            pageNumber: page,
    ...
    const handleUTRCardClick = useCallback(
        (utr: string) => {
    ...
    const dateRangeText = () => {
        ...
        const isDateFilterApplied =
            dateClicked && !paymentError && !paymentLoading;
        const isFirstRender =
    ...
    return (
        <div className="payment-container">
            {searchClicked ? (
                ...
                >
                    <ArrowBackIcon className="back-icon" /> {t('pod.showingsr')}
                </div>
            ) : (
                <SelectDate
                    fromDate={fromDate}
                ...
                {paymentLoading ? (
                    ...
                    >
                        <img
                            src={NoInvoice}
                            alt="no-utr"
                            className="no-invoice-img"
                        />
                        <p className="no-invoice-text">
                            {t('payment.noutrfound')}
                        </p>
                    </div>
                ) : (
